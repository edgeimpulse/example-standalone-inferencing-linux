 ...
 *        QNN_TENSORMEMTYPE_RETRIEVE_RAW. Qnn_GetTensorRawDataRn_t may be called multiple times for the same tensor.
 *        Each call to Qnn_GetTensorRawDataRn_t must be accompanied by a call to Qnn_FreeTensorRawDataFn_t
 *        to free any allocated data for that tensor. It is not required that this function be thread safe,
 *        unless needed to support retrieval of tensor resources that may be shared between threads.
 *
 ...
 *         - QNN_TENSOR_ERROR_DOES_NOT_EXIST: Tensor with __id__ does not exist or was not created as QNN_TENSORMEMTYPE_RETRIEVE_RAW.
...
/**
 * @brief A client-defined function used to free tensor data previously obtained by Qnn_GetTensorDataFn_t.
 *        After the call to Qnn_FreeTensorDataFn_t the data provided in the client buffer clientBuf should
 *        be considered invalid. If Qnn_GetTensorRawDataRn_t has been called multiple times for the same tensor
 *        then Qnn_FreeTensorRawDataFn_t must be called an equivalent number of times
 *        to free all allocated data for this tensor. It is not required that this function be thread safe,
 *        unless needed to support releasing of tensor resources that may be shared between threads.
 *
 * @param[in] id The tensor ID.
 * @param[in] context the context to which the tensor is associated
 * @param[in] graph the graph to which the context is associated. For context tensors this field should be
 *            null.
 *
 * @return Error code:
